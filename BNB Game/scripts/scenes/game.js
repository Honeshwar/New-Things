...
export function game() {
    ...
    const bigPieceWidth = window.innerWidth;
    const bigPieceHeight = window.innerHeight;
    ...
    const scoreTextGameObject = k.add([
        k.text("SCORE: " + score, { font: "mania", size: 22 }),
        k.pos(20, 20),
        k.area(),
        k.scale(2)
    ])
    ...
    const sonicGameObject = makeSonic(k.vec2(200, bigPieceHeight / 2 + 50))//x,y
    ...
    k.add([
        k.rect(1440, bigPieceHeight / 3),
        k.pos(0, bigPieceHeight / 1.1),
        k.opacity(0),
        k.area(),
        k.outline(6, k.Color.fromArray([0, 0, 0])),
        k.body({
    ...
    const spawnEnemy = () => {
        const enemy = makeEnemy(k.vec2(1440, bigPieceHeight - 70));
    ...
    const spawnRing = () => {
        ...
        const ring = makeRings(k.vec2(1440, bigPieceHeight - 70));//similar as {x:1950, y:745}
        ring.onUpdate(() => {
            if (gameSpeed < 3000) {
    ...
    k.onUpdate(() => {
        ...
        if (chemicalBg[1].pos.x < 0) {
            chemicalBg[0].moveTo(chemicalBg[1].pos.x + chemicalBg[1].width * 1, 0)
            chemicalBg.push(chemicalBg.shift())
        }
        chemicalBg[0].move(-100, 0)
        chemicalBg[1].moveTo(chemicalBg[0].pos.x + chemicalBg[0].width * 1, 0)
